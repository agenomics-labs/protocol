...
pub mod agent_registry {
    ...
    pub fn register_agent(
        ...
        // `__padding_aud007` padding array, which Anchor zero-initializes
        // through the discriminator-init path. No explicit assignment needed.
        agent_profile.__padding_aud007 = [0u8; 17];
    ...
    ///         (AUD-108: rejected at the handler entry until ADR-bumped)
    ///
    ...
    ///
    /// AUD-112 (cycle-2 transitional-window note): for any pre-migration
    /// profile that still carries an unbounded legacy `reputation_score`
    /// (> MAX_REPUTATION_SCORE), this handler clamps `old_score` into
    /// the `[0, MAX_REPUTATION_SCORE]` window for arithmetic and writes
    /// the resulting in-range value back. After the first call the
    /// stored field is in-range. Off-chain readers that load the
    /// profile *before* any post-migration call observe the legacy
    /// score; SDK consumers that need policy-conformant values across
    /// the migration window should clamp on read with
    /// `Math.min(reputation_score, MAX_REPUTATION_SCORE)`. The race
    /// window is narrow (any call self-heals) and `migrate_agent_profile`
    /// is the durable normalization path.
    ...
    pub fn migrate_agent_profile(ctx: Context<MigrateAgentProfile>, target_version: u8) -> Result<()> {
        ...
        // bytes they occupied are now `__padding_aud007: [u8; 17]` — a
...
mod tests {
    ...
    fn aud_007_register_agent_zero_initializes_reserved_padding() {
        let p = fixture_profile(0, AgentStatus::Active, 0);
        assert_eq!(p.__padding_aud007, [0u8; 17],
    ...
    /// `profile.__padding_aud007 = [0u8; 17];` so a pre-migration profile
    ...
    fn aud_007_migrate_zeros_reserved_padding_from_stale_state() {
        ...
        p.__padding_aud007 = [0xAB; 17];
        // Mirror the migrate_agent_profile assignment.
        p.__padding_aud007 = [0u8; 17];
        assert_eq!(p.__padding_aud007, [0u8; 17],
