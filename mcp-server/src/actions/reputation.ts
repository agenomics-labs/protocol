...
> = {
  ...
  description:
    "Fetches the merged reputation snapshot for an agent: on-chain Registry native state (reputation_score, stake, slash_count, status, avg_rating, total_tasks_completed) + capability manifest summary (fetched from IPFS and validated via @aeap/capability-manifest-validator) + optional SAS attestation signal (resolved via @aeap/sas-resolver). Read-only — no on-chain writes.",
