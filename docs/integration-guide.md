...
The `@agenomics/integrations` package exports an ElizaOS plugin that wraps all 25 MCP tools as ElizaOS actions.
...
The plugin registers 25 actions prefixed with `aep_`:

- **Vault (9):** `aep_create_vault`, `aep_get_vault_info`, `aep_vault_transfer`, `aep_update_vault_policy`, `aep_rotate_agent_identity`, `aep_pause_vault`, `aep_resume_vault`, `aep_manage_allowlist`, `aep_vault_token_transfer`
- **Registry (5):** `aep_register_agent`, `aep_get_agent_profile`, `aep_update_agent_profile`, `aep_discover_agents`, `aep_stake_reputation`
- **Reputation (1):** `aep_get_agent_reputation`
...
Once configured, Claude can directly call any of the 25 AEP tools to create vaults, discover agents, manage escrows, and handle payments.
