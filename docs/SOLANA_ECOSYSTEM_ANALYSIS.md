...
**Repos analyzed**: 22 (triaged by AEAP relevance — **not stars** — per directive)
...
**The Solana Foundation already ships ~60% of what AEAP was planning to design from scratch.** This scan collapses the scope of ADR-058/059/060 significantly: most of the "signer trait", "tx pipeline", "token handling", and "wire format" work becomes dependency adoption rather than bespoke design.
...
**The real AEAP-unique surface is narrower than expected**:
...
The solana-labs org is **95% archived** (mass-archive date 2026-03-09) — active development moved to `anza-xyz` (Firedancer/Agave). Only two archived-but-canonical reference programs matter for AEAP: `perpetuals` (custody PDA patterns) and `launchpad` (phased-release patterns).
...
| `mosaic` | Token-2022 mint creation (metadata, permanent-delegate, default-account-state, confidential-transfers, transfer-fee, scaled-ui-amount) | ADOPT for Vault asset mints; **gap: no TransferHooks wrapper — AEAP builds that** |
| `mpp-sdk` | Canonical Solana HTTP-402 wire format (`PaymentChallenge` + `PaymentCredential`, canonical JSON, base64url, challenge echo, splits up to 8, fee_payer mode, session mode) | **If AEAP settlement speaks x402, conform — don't invent** |
...
### 2.1 `kora` authorization model → AEAP vault policy engine

`kora` is a Solana relayer with a production-grade authorization stack. Port these patterns into AEAP's Vault program:
...
AEAP's Vault daily cap + per-recipient limit + token allowlist maps directly onto this shape.
...
### 2.3 `commerce-kit` payment state machine → AEAP milestone state machine

`commerce-kit` ships a production payment escrow with clean state transitions:

| commerce-kit status | AEAP milestone status |
|---|---|
...
### 2.4 `fiber` two-phase settlement → AEAP escrow finalization
...
**Open decision**: build AEAP escrow **on top of fiber** (use its primitive, add milestone/dispute layers) vs. **diverge** (build bespoke milestone escrow). Pushed to ADR-059.

### 2.5 `governance-program-library` voter PDA → lightweight dispute resolver

Heavyweight Anchor governance isn't right for AEAP, but the **voter PDA structure** is:
...
Port as lightweight AEAP `DisputeResolver` program. Don't depend on the full governance library.

### 2.6 `perpetuals` per-user position PDA → AEAP Vault custody

The canonical custody pattern for multi-user programs:
- **Global program account**: `Perpetuals` / AEAP `Vault` config
- **Per-pool account**: `Pool` / AEAP asset-class config
...
Primitives where AEAP should **wrap** the Foundation primitive as a substrate layer, keeping AEAP's own semantics on top.

### 3.1 `solana-attestation-service` (SAS) — third-party reputation

**SAS does NOT replace AEAP's Agent Registry.** SAS is a credential issuance system (schema + attestation + authorized-signer); AEAP Registry is identity + state + governance. Hybrid approach:

- **AEAP Registry**: keeps internal state (tier, delegations, active vaults, capability manifests, in-program revocation governance)
- **SAS layer on top**: third-party attestations about agents (KYC, behavioral scores, cross-party reputation)

SAS schema seeds: `["schema", credential, name, version]`; attestation seeds: `["attestation", credential, schema, nonce]`. AEAP can publish an "Agent Manifest" schema and reference SAS attestations from the Registry.

### 3.2 `connectorkit` — wallet enumeration, upstream of signing

`connectorkit` is a Wallet-Standard-based session manager — **upstream** of signing. When AEAP's MCP returns an unsigned tx, the client uses `connectorkit` to enumerate available wallets, pick one, and produce the signature. It complements `@solana/keychain-core` (which is the signer trait), not replaces it.

**Integration**: AEAP MCP returns `{ type: "unsigned_transaction", serialized_tx, required_signers[] }` → client's `connectorkit` session resolves signers → signs via `SolanaSigner` backends → submits.

---

## Tier 4 — Conform to shape

Where AEAP should conform to the Foundation's canonical shape rather than invent its own.

### 4.1 `solana-mcp-official` `SolanaTool` shape → AEAP MCP tool interface
...
AEAP's `Action<I, O>` shape (ADR-058) should be a **strict superset** of this:
...
interface Action<I, O> extends SolanaTool {
  ...
  preflight?: PreflightGate[];        // AEAP addition
  // inputSchema === parameters, outputSchema unchanged
}
```

**Critical divergence**: `solana-mcp-official` is read-only with zero auth. AEAP ships state-mutating operations (vault deposit, settlement release, dispute resolution) and **must** add the capability-gating layer that `solana-mcp-official` deliberately omits. Reject sendaifun's hot-keypair shortcut.

### 4.2 `solana-dev-skill` Markdown+YAML → AEAP skill format
...
**AEAP skill distribution**: publish `skills/aeap/{vault-initialization,settlement-lifecycle,dispute-resolution}.md` in **both** repos — `solana-dev-skill` is the format authority, `sendaifun/skills` is the distribution channel.
...
| `txtx` | HCL deployment runbooks — a different abstraction layer than ADR-060's capability descriptor (runbook vs. capability contract). Reference action-request pattern only if AEAP adds multi-sig consent flows. |
| `solana-record-service` (SRS) | Too generic; AEAP needs agent-specific semantics (tier, delegation, vault bindings). Could reuse dual-ownership pattern (Pubkey vs. Token) but not the program itself. |
| `moneymq` | Stripe-webhook bridge; relevant only if AEAP adds fiat onramps. No new primitives over `mpp-sdk` for pure on-chain flows. |
| `solana-payments-app` (solana-labs, archived) | Legacy Solana Pay on `@solana/web3.js` v1. Superseded by `mpp-sdk`. |
| `pay` (solana-foundation) | Useful split-resolution semantics (percent-of-original, not remaining-balance) — but AEAP likely builds its own escrow. Reference the split model only. |
...
|---|---|
| `solana-mcp-official` zero-auth model | AEAP ships state-mutating ops; read-only-with-no-auth is wrong for settlement. Add capability-gating. |
...
| Full `governance-program-library` dependency | Anchor-heavyweight for AEAP's use case; port the voter PDA shape only. |
...
| Layer | ADR was going to design | Foundation ships | AEAP's remaining work |
|---|---|---|---|
...
| Agent identity | Build Registry alone | `SAS` for reputation | AEAP Registry keeps internal state; SAS wraps as reputation substrate |
...
**Rough estimate**: ~60% of the original ADR-058/059/060 design work collapses to dependency adoption. The remaining ~40% is AEAP-unique: capability taxonomy + capability-gating + milestone state machine + dispute flow + cross-program CPI coordination.
...
7. **Fiber vs. custom escrow** — build AEAP Settlement on top of `fiber` or build bespoke? Fiber's 42-byte account + Blake3 distribution-hash is elegant; AEAP's milestone/dispute semantics may need richer state.
...
10. **mpp-sdk canonical conformance** — does AEAP settlement speak HTTP-402 as a wire format (conform to `PaymentChallenge` / `PaymentCredential`) or is AEAP purely on-chain (Anchor IX calls only)? If any agent-to-agent flow is HTTP-mediated, must conform.
...
- **AEAP verdict**: **REFERENCE** for split semantics. Likely build own escrow.
...
- **AEAP verdict**: **ADOPT-AS-DEP**. Canonical Solana x402. AEAP agents should conform.
...
- **AEAP verdict**: **REFERENCE / OPEN DECISION** (build on top vs. diverge).
...
- **AEAP verdict**: **REFERENCE for Vault authorization model.**
...
- **AEAP verdict**: **SUBSTRATE** (reputation wrapper; AEAP Registry keeps internal state).

#### `solana-record-service` — ignore
Generic record store. Class + Record PDAs, dual ownership (Pubkey or Token). Too broad for AEAP's agent-specific needs. Reference dual-ownership pattern only.
...
- **Pattern**: `freeze_authority` points at a pluggable gating program — port as AEAP Vault → Policy Program indirection.
...
- **AEAP verdict**: port PDA structure for asset-allowlist components.
...
- **AEAP verdict**: **ADOPT** for Vault mint creation. **Build TransferHooks wrapper separately.**
...
- **AEAP verdict**: port voter PDA shape; don't depend (Anchor-heavyweight).
...
- **AEAP verdict**: **ADOPT-AS-DEP for ADR-058**. Replaces the planned BaseWallet trait entirely.
...
- **AEAP verdict**: **EXTEND** as client-side wallet enumeration (upstream of `keychain-core` signing).
...
- **AEAP verdict**: **ADOPT the blockhash cache + compute-budget helpers**. Skip React hooks (AEAP headless).
...
- **AEAP verdict**: **DIVERGE** from ADR-060 (different abstraction). Reference action-request pattern only for multi-sig consent flows.
...
- **AEAP verdict**: **CONFORM to tool shape, DIVERGE on signing** (add capability-gating layer).
...
- **AEAP verdict**: **PORT format for AEAP skills**. Publish `skills/aeap/{vault-initialization,settlement-lifecycle,dispute-resolution}.md` in `sendaifun/skills` distribution.
...
- **AEAP verdict**: **PORT state machine + PDA seed pattern + settlement-delay validation**.
...
- **AEAP verdict**: **ADOPT-AS-DEP**. Eliminates hand-rolled ATA derivation.
...
- **AEAP verdict**: **REFERENCE** for multi-user position PDA pattern + per-asset custody lifecycle. Skip leverage/liquidation.
...
- **AEAP verdict**: **FUTURE REFERENCE** for phased/tiered milestone settlement if AEAP adds vesting or tranched releases.
...
- PR: adopt `framework-kit` blockhash/compute-budget helpers; add mutex-per-sig; port commerce-kit Paid→Cleared→Refunded to AEAP milestone state machine.

### Week 4 — Distribution
- PR `@aeap/plugin-solana-agent-kit` alpha (D-1 from sendaifun analysis).
- PR `skills/aeap/*.md` to both `sendaifun/skills` and reference the `solana-dev-skill` format.
- PR `solana-new` CLI index entry.

### Week 5+ — Protocol alignment stretch
- `mpp-sdk` conformance if AEAP speaks x402.
