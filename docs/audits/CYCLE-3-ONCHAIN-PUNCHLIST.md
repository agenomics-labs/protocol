...
| ID | Title | File:Lines | Owner | Status |
|---|---|---|---|---|
| AUD-201 | Stuck-Active escrow refund gap — provider accepts then abandons; only refund path is `expire_escrow` after 365-day deadline. With dispute timeout the worst-case lock is up to 730 days. `cancel_escrow` is `Created`-only. | `programs/settlement/src/instructions/escrow.rs:360-410` | k2jac9 | **Closed — `<pending>`** [^aud201] |
...
- **High**: AUD-201 ✅ closed `<pending>` (mutual-rescission `cancel_active_escrow`); AUD-202 ✅ closed `e59072a` (Settlement-side build-time field-order pin)
