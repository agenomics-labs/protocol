...
| AUD-211 | L   | **Closed** | `wrap` removed from the `mcp-server/src/util/result.ts` re-export list. The canonical action-runtime `wrap` returns `Result<T, Error>`; every mcp-server action that needs a try/catch wrapper ships its own local `wrap` returning `Result<T, AepError>` with `code: "PROGRAM_ERROR"`. Removing the re-export prevents future drift (a contributor importing from this module would no longer be tempted to use the AepError-incompatible canonical shape). Comment above the export documents the rationale and points at the cycle-3 follow-up (single shared AepError-shaped wrap across `actions/*`). `npm run build --workspace @agenomics/mcp-server` clean. |
| AUD-212 | L   | **Closed (doc)** | Threat-model decision documented inline at `mcp-server/src/pipeline/idempotency-redis.ts` `deserializeResult`. Rationale: Redis is inside the trust boundary (operator-controlled, network-isolated per ADR-117). Tampering requires Redis-write access, which already implies the boundary has been breached. The HMAC-add (keyed by an env-supplied secret) is the documented enforcement-point IF a future deployment moves Redis outside that boundary (shared cluster, multi-tenant); tracked alongside ADR-117. |
