...
| AUD-406 | M   | CI flakes are ~60% of CI failure volume; no mitigation shipped | **Closed (2026-04-26).** All three mitigation tiers shipped: (1) `scripts/seed-runner-action-cache.sh` (auto-discovers `uses:` refs, fetches each tarball once via gh api, mirrors to every `/home/neo/actions-runner*/_work/_actions/<owner>/<repo>/<ref>/`; idempotent, dry-run support) — committed in `a2e5d6d`. (2) Every external `uses:` ref SHA-pinned (45 sites across 6 workflow files; ADR-114 dependabot config already covers the `github-actions` ecosystem so SHAs auto-bump weekly) — same commit. (3) `trufflesecurity/trufflehog` vendored as a path-based composite action at `.github/actions/trufflehog/` (eliminates api.github.com fetch for the only `@main`-tracking action) — committed in `bf12506`. All 4 self-hosted runners seeded; 28 SHA-keyed cache entries verified post-PR. |
