...
| AUD-406 | M   | CI flakes are ~60% of CI failure volume; no mitigation shipped | **Mitigated (2026-04-26).** Hybrid per ADR-123: (Tier 2) every external `uses:` ref SHA-pinned, 45 sites across 6 workflow files, Dependabot-trackable via the existing `github-actions` ecosystem entry from ADR-114 — `a2e5d6d`. (Tier 3, selective) `trufflesecurity/trufflehog` vendored as `./.github/actions/trufflehog/` — `bf12506`; the only fully-closed action (Secret Scan job hits zero api.github.com fetches, validated on the run that landed `bf12506`). (Tier 1, bookkeeping) `scripts/seed-runner-action-cache.sh` shipped — `a2e5d6d`. **Limitation discovered post-rollout**: `actions/runner` v2.334.0 unconditionally re-downloads `<owner>/<repo>/<ref>/` into `_temp_<uuid>/` regardless of pre-seeded content (worker log: `[INFO ActionManager] Save archive 'https://api.github.com/...' into _temp_<uuid>/...`). Tier 1 is therefore a bookkeeping/runbook artifact, not a flake-prevention tool. The remaining six SHA-pinned actions retain a residual flake surface; the accepted response is the AUD-406 rerun reflex (`gh run rerun <id> --failed`). If residual flake rate proves unacceptable, escalation path is to extend Tier-3 vendoring across the remaining six actions using `.github/actions/trufflehog/` as the template — tracked in ADR-123 §Consequences. |
...
| AUD-409 | L   | Stale Node 20 deprecation warnings on every run                | **Closed (2026-04-26).** Bumped to current Node-24-runtime majors: `setup-node@v4→v6` (11 sites), `cache@v4→v5` (8 sites), `download-artifact@v4→v8` (2 sites), `upload-artifact@v4→v7` (2 sites). Self-hosted runner verified at v2.334.0 (≥ the 2.327.1 floor required by `cache@v5` and `setup-node@v6`). `download-artifact@v8` defaults `digest-mismatch=error` (secure-by-default); same-CI source/sink so no behavior impact. |
