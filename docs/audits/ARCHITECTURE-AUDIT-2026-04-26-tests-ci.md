...
| ------- | --- | -------------------------------------------------------------- | -------------- |
| AUD-400 | M   | `mainnet-readiness.yml` checklist regex is `Pending`-only      | Extend to `\| (Pending\|TBD\|Partial\|In Progress\|InProgress\|Blocked\|WIP) \|`, OR enforce a status-cell schema in MAINNET_CHECKLIST.md (only `Done` and `N/A` are pass values; everything else fails). Today's corpus has no live false negative, but the regex is brittle to future status-vocabulary additions. |
...
| AUD-404 | M   | AUD-055 (wall-clock waits) — two stragglers persist            | (a) `tests/settlement.ts:2152` AUD-009 still has `await new Promise((resolve) => setTimeout(resolve, 4000))`. Replace with the slot-poll pattern from PR-L (`while (Date.now() < pollDeadline) { await connection.getSlot('confirmed') … }`). (b) AUD-018 grace-elapsed case has a 30s polling deadline as defence-in-depth — non-blocking but noisy. Tracked, not a blocker. |
...
5. **AUD-404**: fix the AUD-009 wall-clock straggler in
   `tests/settlement.ts:2152` using PR-L's slot-poll pattern.
