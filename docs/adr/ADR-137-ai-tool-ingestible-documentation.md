...
This ADR sits alongside ADR-134 (Codama-generated SDK; produces
...
- One pair (`CLAUDE.md` symlinked to `AGENTS.md` for cross-vendor
  compatibility) at the root of each public package:
  - `sdk/idl/CLAUDE.md`
  - `sdk/client/CLAUDE.md`
  - `sdk/action-runtime/CLAUDE.md`
  - `packages/capability-manifest-validator/CLAUDE.md`
...
- Format: ~200–500 lines of structured prose **for the LLM, not the
  ...
  - Pointers to the typed shape: which Zod schema (ADR-135), which
    Codama builder (ADR-134), which IDL field.
...
#### 3. Repo-root `.cursorrules`

- One small file at the repo root (~50–80 lines) that captures the
  protocol-wide conventions any AI-assisted contributor should
  follow: branch naming, ADR discipline, IDL parity, conventional
  commits, "no Co-Authored-By trailers" (per the existing
  `.git/hooks/commit-msg` policy), Zod schemas as the boundary
  contract, Codama as the source of truth.
- Cursor reads `.cursorrules` automatically. Claude Code reads
  `CLAUDE.md` (already present). The two files do not duplicate —
  `.cursorrules` is contributor-facing repo discipline; `CLAUDE.md`
  is per-package consumer-facing guidance.
...
- We do **not** ship a hosted MCP server *for the docs themselves*.
  That's a separate decision (potentially an ADR-141+) and risks
...
- **Closes the AI-conversion gap.** A dev asking Cursor "register
  ...
  because the model has the IDL shape (via Codama, ADR-134), the
...
- **Reinforces the codegen story.** When ADR-134's CI gate fails
...
- ADR-134 — Codama-generated clients (the typed surface this ADR
  documents in prose).
