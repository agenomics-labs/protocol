...
- **Codegen from spec, not hand-rolled.** ADR-134 lands codegen on
...
- A drift gate (`scripts/check-mcp-tool-schemas.sh`, wired into
  ...
  ADR-082 IDL coverage gate and ADR-134 Codama diff gate).
...
generated builder's input shape** from ADR-134. The Zod schema is the
...
- **One more drift gate to maintain.** Acceptable — the ADR-082 +
  ADR-134 + this ADR triplet establishes the pattern that
...
- ADR-134 — Codama-generated Anchor clients; pairs with this ADR
